...
CONFIG = {
    "LOAD_FROM_CSV": True,
    "POSITIVE_CSV": "tagged_erm.csv",
    "UNLABELED_CSV": "unlabeled_filtered_cme_neuro_retina_amd_cnv.csv",
    ...
    "OUTPUT_SCORES_CSV": "unlabeled_scores_erm.csv",
    "OUTPUT_FLAGGED_CSV": "unlabeled_flagged_erm.csv",
    "ARTIFACT_MODEL": "best_model_erm.joblib",
    "ARTIFACT_JSON": "model_meta_erm.json",
}
...
def score_and_save_unlabeled(best: Dict[str, Any],
    ...
    drop_cols = keep_id_cols + [c for c in ["is_erm_labeled", "y_supervised"] if c in df_unl.columns]
    ...
    scores_df["p_erm"] = p_true_unl
    scores_df["flag_erm_like"] = flags
...
def prepare_missingness(df_pos, df_neg, df_unl, id_cols, ts_col):
    summarize_missing(df_pos, "ERM (positives)")
...
def main(df_pos: pd.DataFrame = None, df_unl: pd.DataFrame = None,
    ...
    # PU setup
    df_pos_pu = df_pos.copy(); df_pos_pu["is_erm_labeled"] = 1
    df_unl_pu = df_unl.copy(); df_unl_pu["is_erm_labeled"] = 0
    df_all_pu = pd.concat([df_pos_pu, df_unl_pu], ignore_index=True)
    num_cols_pu, cat_cols_pu = infer_feature_lists(df_all_pu, id_cols, ["is_erm_labeled"])
    preproc_pu = make_preprocessor(num_cols_pu, cat_cols_pu)
    Xtr, Xte, ytr, yte = time_or_random_split(df_all_pu, "is_erm_labeled", ts_col, test_size=0.3, seed=42)
    ...
    score_and_save_unlabeled(
        best_res, df_unl, id_cols,
